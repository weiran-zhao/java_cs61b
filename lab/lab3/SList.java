/* SList.java */
// Modified by Ryan (Weiran) Zhao to finish lab3
// Tue,Jun 11th 2013 09:52:13 AM EDT
//
// History:
// code commented with ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// are for new insertEnd() function
...
public class SList {
  ...
  private int size;
  // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  // add to maintain tail to provide constant-time insertEnd
  // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  public static void main (String[] args) {
    ...
    System.out.println("=========Testing Insert List==========");
    SList lst = new SList();
