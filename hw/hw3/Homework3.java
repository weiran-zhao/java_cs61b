...
// Tue,Jun 11th 2013 05:18:42 PM EDT
...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
      /** 
       *  The basic idea is for an array of length n, if the i-th element
       *  is the same as (i-1)th; we move i~n to (i-1)~n; then check again
       *  at i-th element
       *  if i-th element is different from (i-1)th, or it is '-1', meaning 
       *  we reach the end; we stop
       */ 
      // we start from 1, the first element must stay where it was
      int i=1;
      int endidx=ints.length; // signify the end of original array
      while(i<ints.length) {
          ...
          // reach the end of original array
          if((ints[i]==-1) || (i==endidx))
              break;
          // moving and fill in '-1' for trailing spots
